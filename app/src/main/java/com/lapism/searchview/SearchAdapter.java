...
public class SearchAdapter extends RecyclerView.Adapter<SearchAdapter.ResultViewHolder> implements Filterable {
    ...
    public List<SearchItem> typeAheadData = new ArrayList<>();
...
//                }
                ...
                FilterResults filterResults = new FilterResults();
                filterResults.values = typeAheadData;
                filterResults.count = typeAheadData.size();
...
//                }
    ...
    public void onBindViewHolder(ResultViewHolder viewHolder, int position) {
        ...
        SearchItem item = typeAheadData.get(position);
        int start = position<startList.size()?startList.get(position):0;
        ...
        if (theme == 0) {
            ...
            try {
                ...
                s.setSpan(new ForegroundColorSpan(ContextCompat.getColor(mContext, R.color.search_light_text_highlight)),
                        start, end<lenght?end:(lenght-1), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        if (theme == 1) {
            ...
            try {
                ...
                s.setSpan(new ForegroundColorSpan(ContextCompat.getColor(mContext,
                        R.color.search_dark_text_highlight)), start, end<lenght?end:(lenght-1), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
